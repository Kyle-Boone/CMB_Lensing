...
patch_inds = patch_inds_all[np.where(weights>0.1)]
...
def fid_func(map_dir):
    ...
    nn_cmb = make_CMB_noise(cmb_survey)
    nn_wl = make_Euclid_noise()
    
    cmb = cmb_map_noiseless + nn_cmb
    if cmb_survey == 'ACT':
        alm = hp.map2alm(cmb, lmax=2090) # This is the max ell I had access to.
        cmb = hp.alm2map(alm, nside)
    if cmb_survey == 'Planck':
        cmb = hp.sphtfunc.smoothing(cmb, fwhm=fwhm_rad)
    low_z = low_z_noiseless + nn_wl
    high_z = high_z_noiseless + nn_wl
    
    cmb[np.where(total_mask < 2.5)] = 0
    low_z[np.where(total_mask < 2.5)] = 0
    high_z[np.where(total_mask < 2.5)] = 0
    
    all_wph = WPH_maps(cmb, low_z, high_z)
            
    np.save(write_file, all_wph)
...
if __name__ == "__main__":   
    ...
    with Pool(50) as pool: # 400 GB, 6 hours
