...
unmask = []
for survey in surveys:
    ind_weights = np.load(patch_dir + 'weights_' + survey + '.npy')
    ind_patch_inds = all_patch_inds[np.where(ind_weights>0)]
    ind_unmask = ind_patch_inds > -0.5
    patch_inds.append(ind_patch_inds)
    unmask.append(ind_unmask)
...
def WST_map(survey_map, ind):
    ...
    patches = np.zeros_like(patch_inds[ind]).astype(float)
    patches[unmask[ind]] = np.take(survey_map, patch_inds[ind][unmask[ind]])
...
def fid_func(file):
    ...
    for survey_ind in np.arange(len(surveys)):
        ...
        wst = WST_map(cmb_map, survey_ind)
        
        data_dict[survey] = wst
    ...
    print(f"The method executed in {elapsed_time_minutes:.2f} minutes.")
    return None
...
def deriv_func(file):
    ...
    for survey_ind in np.arange(len(surveys)):
        ...
        wst_p = WST_map(cmb_map_p, survey_ind)
        wst_m = WST_map(cmb_map_m, survey_ind)
...
if __name__ == "__main__":   
    ...
    test_files = fid_files

    with Pool(75) as pool:
