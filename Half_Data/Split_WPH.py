...
num_Derivs = 25
surveys = ['ACT_x_dr3', 'Planck_x_dr3', 'SO_x_dr3', 'SPT_x_dr3']
for survey in surveys:
    ...
    x_pivots = np.load('/n/home09/kboone/software/Data/MCMC_Sorting/WPH_J7_L4_Pivots.npy')
    len_y = len(x_pivots)
    ...
    write_file = piv_dir+'WPH_'+str(num_Derivs)+'_'+survey+'.npy'
    ...
    for i in np.arange(num_Derivs):
        all_partial_Om[i] = (np.load(deriv_dir + all_Om_files[i], allow_pickle=True)[:data_len].real)[sortInds]

    all_partial_Om = all_partial_Om / std_fid

    all_partial_s8 = np.zeros((num_Derivs, data_len), dtype = np.float64)
    for i in np.arange(num_Derivs):
        all_partial_s8[i] = (np.load(deriv_dir + all_s8_files[i], allow_pickle=True)[:data_len].real)[sortInds]
