...
# REMEMBER TO CHANGE SPLITS FILE AND WRITE FILE
num_Derivs = 25
...
for survey in surveys:
    ...
    x_inserts = np.load('/n/home09/kboone/software/Data/MCMC_Sorting/WST_S50_L4_Pivots.npy')
    ...
    write_file = piv_dir+'WST_'+str(num_Derivs)+'_'+survey+'.npy'
    ...
    for i in np.arange(num_Derivs):#len(all_partial_Om)):
    ...
    for i in np.arange(num_Derivs):#len(all_partial_s8)):
    ...
    def find_pivots(realization):
        ...
        def eval_f(all_pivots):
            ...
            x_pivots = np.append(x_pivots, x_inserts)
        ...
        x_params = np.append(x_params, x_inserts)
